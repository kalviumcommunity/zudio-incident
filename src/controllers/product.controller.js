...
const getProducts = async (req, res) => {
  try {
    ...
    if (search) {
      // search by name — parameterised to prevent SQL injection
      const s = `%${search}%`
      result = await pool.query(
        'SELECT p.*, c.name as category_name FROM products p JOIN categories c ON p.category_id = c.id WHERE p.name LIKE $1 ORDER BY p.created_at DESC LIMIT $2 OFFSET $3',
        [s, parseInt(limit), parseInt(offset)]
      )
    } else if (category) {
      result = await pool.query(
